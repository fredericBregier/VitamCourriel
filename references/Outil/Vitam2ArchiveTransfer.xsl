...
<!--
    Document   : Vitam2ArchiveTransfer
    Created on : 22/09/2012
    Author     : Frederic Bregier
    Description: translate Vitam output of Full Format to ArchiveTransfer model
-->
...
<xsl:template match="/">
	<xsl:if test="$verbose='yes'">
		<xsl:message> Vitam 2 Seda ArchiveTransfer - version <xsl:value-of select="$version"/> starting </xsl:message>
...
<ArchiveTransfer xmlns="fr:gouv:culture:archivesdefrance:seda:v1.0">
	...
	<TransferIdentifier/>
	<ArchivalAgency>
		<Identification>Identification SAE</Identification>
	</ArchivalAgency>
	<TransferringAgency>
		<Identification>Identification Versant</Identification>
	</TransferringAgency>
	
	<Archive Id="IdArchive">
		...
		<AccessRestrictionRule Id="idAccessRestrictionRule">
			<Code>AR038</Code>
			<StartDate><xsl:value-of select="substring(@date,1,10)" /></StartDate>
