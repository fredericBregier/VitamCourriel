    ...
    <pattern>
        <rule context="seda:ContentDescription[(seda:LatestDate) and (seda:OldestDate)]">
            ...
            <assert test="number($endDate) >= number($startDate)">
                Erreur: date de fin <value-of select="seda:LatestDate"/> est postérieure à la date de début <value-of select="seda:OldestDate"/>
            </assert>
        </rule>
    </pattern>
   <pattern>
        <rule context="seda:LatestDate">
            ...
            <assert test="($ancestorEndDate = '') or (($endDate) &lt;= ($ancestorEndDate))">
                Erreur: date de fin <value-of select="."/> est antérieure à la date de fin d'un niveau parent <value-of select="parent::seda:ContentDescription/../ancestor::seda:*[seda:ContentDescription/seda:LatestDate][1]/seda:ContentDescription/seda:LatestDate"/>
            </assert>
        </rule>
    </pattern>
    ...
    <pattern>
        <rule context="seda:OldestDate">
            ...
            <assert test="($ancestorStartDate = '') or (($startDate) >= ($ancestorStartDate))">
                Erreur: date de début <value-of select="."/> est postérieure à la date de début d'un niveau parent <value-of select="parent::seda:ContentDescription/../ancestor::seda:*[seda:ContentDescription/seda:OldestDate][1]/seda:ContentDescription/seda:OldestDate"/>
            </assert>
        </rule>
    </pattern>
